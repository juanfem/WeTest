...
def value_from_subtest(key, test_infos, scenario_id, test_id,
    ...
    if subtest_id is None:
        value = getattr(list(test_infos[scenario_id][test_id].values())[
                        0], key, fallback)
    else:
        value = getattr(test_infos[scenario_id]
                        [test_id][subtest_id], key, fallback)
    return value
...
class GUIGenerator(object):
    def __init__(self, master,
        ...
        self.footer_frame = tk.Frame(
            self.master, borderwidth=1, relief="raised")
        ...
        self.stop_button = tk.Button(self.buttons_frame, text="Abort".center(BT_TXT_LEN), command=self.abort,
                                     compound="left", image=self.button_img["stop"], state="disable")
        self.stop_button.pack(side="left")
        Tooltip(self.stop_button, text="Abort testing\nNo report generated")

        self.report_button = tk.Button(self.buttons_frame, text="Report".center(BT_TXT_LEN), command=self.report,
        ...
        self.close_button = tk.Button(self.buttons_frame, text="Quit".center(BT_TXT_LEN), command=self.quit,
    ...
    def pause(self):
        ...
        if self.playing:
            ...
            for sc in self.suite_gui.status_children:
                sc.set_children_status(
                    status=STATUS_PAUSE, dynamic=False)
            ...
            if not self.update_queue.empty():
                ...
                if update in [END_OF_TESTS, ABORT_FROM_MANAGER]:
                    ...
                    if update == ABORT_FROM_MANAGER:
                        ...
                        for sc in self.suite_gui.status_children:
                            sc.set_children_status(
                                status=STATUS_STOP, dynamic=False)
                    ...
                    if test_status in [STATUS_RUN, STATUS_RETRY]:
                        ...
                        self.current_test_retrying = (
                            test_status == STATUS_RETRY)
                    else:
                        self.current_test_id = None
                        self.current_test_retrying = False
...
class PopUp(object):
    ...
    def __init__(self, root, gui, title, message, width=None, height=None, centered=True):
        ...
        self.ok_button = tk.Button(
            self.buttons_frame, text="Ok".center(BT_TXT_LEN), command=self.on_close,
        ...
        if centered:
            ...
            position = "+%d+%d" % (root_x + old_div(root_w, 2) -
                                   old_div(top_x, 2), root_y + old_div(root_h, 3))
...
if __name__ == "__main__":  # tests
    ...
    suite.tests_infos["subtest01_0-1"] = {"id": "test_1", "desc": "sc 0 test 1 subtest 1",
                                          "subtest_message": None, "setter": None, "getter": None}
    suite.tests_infos["subtest02_0-1"] = {"id": "test_2", "desc": "sc 0 test 1 subtest 2",
                                          "subtest_message": None, "setter": None, "getter": None}
    suite.tests_infos["subtest01_0-2"] = {"id": "test_3", "desc": "sc 0 test 1 subtest 1",
                                          "subtest_message": None, "setter": None, "getter": None}
    suite.tests_infos["subtest01_1-0"] = {"id": "test_4", "desc": "sc 1 test 0 subtest 1",
                                          "subtest_message": None, "setter": None, "getter": None}
    suite.tests_infos["subtest01_1-2"] = {"id": "test_5", "desc": "sc 1 test 2 subtest 1",
                                          "subtest_message": None, "setter": None, "getter": None}
    suite.tests_infos["subtest02_1-0"] = {"id": "test_6", "desc": "sc 1 test 0 subtest 2",
                                          "subtest_message": None, "setter": None, "getter": None}
    suite.tests_infos["subtest03_1-0"] = {"id": "test_7", "desc": "sc 1 test 0 subtest 3",
                                          "subtest_message": None, "setter": None, "getter": None}
    ...
    GUIGenerator(
        master=root,
        suite=suite,
        configs=["suite title", {'name': "scenario 1 title"}, {
